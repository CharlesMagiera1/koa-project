import * as Router from 'koa-router';
import { TaskService } from '../services/taskservice';
import { InMemoryTaskRepository } from '../repositories/tasks/inmemorytaskrepository';
import { Task } from '../models/tasks/task';

const router = new Router();
const service = new TaskService(new InMemoryTaskRepository());

router.get('/tasks', async (ctx) => {
    console.log('Entry point for request')
    const tasks = await service.getAllTasks();
    console.log('Third - returns from the router')

    ctx.status = 200;
    ctx.body = tasks;

});

router.get('/tasks/:id', async (ctx) => {
    const tasks = await service.getTask(Number(ctx.params.id));

    if (tasks) {
    ctx.status = 200;
    ctx.body = tasks;

    } else { 
        ctx.throw(404);
    }

});

router.post('/tasks', async (ctx) => {
    const task = <Task>ctx.request.body;
    const newTask = await service.createTask(task);

    ctx.status = 201;
    ctx.body = newTask;
    ctx.set('location', 'http://localhost:3000/tasks/' + newTask.id)
});

router.put('/tasks/:id', async (ctx) => {
    const task = <Task>ctx.request.body;
    const updatedTask = await service.updateTask(Number(ctx.params.id), task);

    if(updatedTask) {
        ctx.status = 200;
        ctx.body = updatedTask;
    }else {
        ctx.throw(404);
    }
});

router.delete('/tasks/:id', async (ctx) => {
    const deleteTask = await service.deleteTask(Number(ctx.params.id));

    if(deleteTask) {
        ctx.status = 200;
        ctx.body = deleteTask;
    }else {
        ctx.throw(404);
    }
});

export default router;